...
@Route('admin/faqs')
@Tags('Admin FAQs Management')
@Security('jwt', ['admin'])
export class AdminFaqController extends Controller {
  constructor(private faqService: FaqService = new FaqService()) {
    super();
  }

  /**
   * Admin-only listing of ALL FAQs (including ACTIVE and DRAFT).
   */
  @Get('')
  async getAll(
    ...
    return { data, pagination, message: 'Admin FAQs retrieved successfully', success: true };
  }
  ...
  async getById(@Path() id: string): Promise<ApiResponse<Faq>> {
    ...
    return { data, message: 'Admin FAQ retrieved successfully', success: true };
  }
