...
@Tags('Admin Testimonials Management')
@Security('jwt', ['admin'])
export class AdminTestimonialController extends Controller {
  constructor(private testimonialService: TestimonialService = new TestimonialService()) {
    super();
  }

  /**
   * Admin-only listing of ALL testimonials (including ACTIVE and INACTIVE).
   */
  @Get('')
  async getAll(
    ...
    return { data, pagination, message: 'Admin testimonials retrieved successfully', success: true };
  }
  ...
  async getById(@Path() id: string): Promise<ApiResponse<Testimonial>> {
    ...
    return { data, message: 'Admin testimonial retrieved successfully', success: true };
  }
