...
export class AdminTeamController extends Controller {
  ...
  /**
   * Admin-only listing of ALL team members (including ACTIVE and INACTIVE).
   */
  @Get('')
  async getAll(
    ...
    return { data, pagination, message: 'Admin team members retrieved successfully', success: true };
  }
  ...
  async getById(@Path() id: string): Promise<ApiResponse<TeamMember>> {
    ...
    return { data, message: 'Admin team member retrieved successfully', success: true };
  }
